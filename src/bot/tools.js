...
async function buscar_comercios({ busca, categoria, bairro, limit = 5 }) {
  ...
  return {
    ...
    comercios: data.map(c => ({
      ...
      link_perfil: `https://zappicidade-painel.vercel.app/c/${c.slug}`
    }))
  }
}
...
async function get_detalhes_comercio({ slug }) {
  ...
  return {
    ...
    link_perfil: `https://zappicidade-painel.vercel.app/c/${data.slug}`,
...
async function buscar_promocoes({ categoria, limit = 5 }) {
  ...
  return {
    ...
    promocoes: data.map(p => ({
      ...
      link:      p.comercios?.slug ? `https://zappicidade-painel.vercel.app/c/${p.comercios.slug}` : null
    }))
  }
}
